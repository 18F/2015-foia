...
def merge_data():
    ...
    for name_usacontacts in usacontacts:
        ...
        if name_usacontacts in all_usa_data.keys():
            usacontacts[name_usacontacts]['description'] = all_usa_data[
                name_usacontacts]['description']
            found = True
        ...
        with open(filename) as f:
            ...
            if yaml_data['name'] in data.keys():
                ...
                if data[yaml_data['name']]['acronym'] != "None":
                    yaml_data['abbreviation'] =\
                        data[yaml_data['name']]['acronym']
                    acronyms += 1

                if 'description' not in yaml_data:
                    yaml_data['description'] = data[yaml_data['name']].get(
        ...
        for internal_data in yaml_data['departments']:
            if internal_data['name'] in data:
                ...
                if data[internal_data['name']]['usa_id'] != '':
                    ...
                    internal_data['usa_id'] = data[
                        internal_data['name']]['usa_id']

                if 'description' not in internal_data.keys():
                    internal_data['description'] = data[
                        internal_data['name']].get(
                            "description", "No Description")
                    descriptions += 1

                if data[internal_data['name']]['acronym'] != "None":
                    internal_data['abbreviation'] = data[
                        internal_data['name']]['acronym']
                    acronyms += 1
